...
export function HomeActiveProjects({ role, email }: HomeActiveProjectsProps) {
  ...
  return (
    <section className="space-y-3">
      <SectionHeader title="Active projects" href={role === "producer" ? "/dashboard?tab=projects" : "/vocalist/orders"} />
      <div className="grid gap-2.5">
        {projects.slice(0, 4).map((order) => {
          ...
          return (
            ...
            >
              <div className="min-w-0">
                <p className="truncate font-medium text-white">{order.trackName}</p>
                <p className="mt-0.5 truncate text-xs text-zinc-400">
                  {role === "producer" ? `with ${order.vocalistName}` : order.producerName ?? "Producer project"}
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <span
                  className={`rounded-full border px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-wide ${getHomeOrderStatusStyle(statusLabel)}`}
                >
                  {statusLabel}
                </span>
                <AnimatedButton
...
function SectionHeader({ title, href }: { title: string; href: string }) {
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <AnimatedButton href={href} variant="secondary" className="rounded-lg px-2.5 py-1 text-[10px]">
        View all
      </AnimatedButton>
    </div>
  );
